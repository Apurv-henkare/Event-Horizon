...
function Player:init()
    self.player = world:newRectangleCollider(70,10,20,33,{collision_class="Player"})
...
function Player:update(dt)
    if self.player.body then  
        local colliders =world:queryRectangleArea(self.player:getX()-15,self.player:getY()+10+6,40,2,{'Platform'}) 
        ...
        if Keyboard_was_Pressed('w') then  
            if self.player.body then
                if self.player.grounded then 
                    self.player:applyLinearImpulse(0,-270)
                end         
            end 
        end  
